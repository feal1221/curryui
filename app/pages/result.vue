<template>
  <div class="flex flex-col justify-center items-center gap-4 mt-4">
    ...
    <UiDialog v-model:open="showDialog">
      <UiDialogTrigger>
        <UiButton variant="secondary"> 分享測驗結果 </UiButton>
      </UiDialogTrigger>
      <UiDialogContent>
        <UiDialogHeader>
          ...
          <UiDialogDescription>
            <div class="flex gap-2">
              ...
              <Threads class="cursor-pointer mt-1.5 ml-1" @click="shareToThreads"/>
              <Ig v-if="isMobile" class="cursor-pointer mt-2 ml-2" @click="downloadImage"/>
            </div>
          </UiDialogDescription>
        </UiDialogHeader>
        <!-- <UiDialogFooter>
...
const shareToFB = async () => {
  try {
    ...
    alert("已為您複製結果文字！請直接在 FB 貼上即可分享。");
    // if (navigator.share) {
...
const downloadImage = async () => {
  await navigator.clipboard.writeText(textToCopy);
  alert("已為您複製結果文字！請直接在 IG 貼上即可分享。");
  // IG 的限時動態跳轉協議
